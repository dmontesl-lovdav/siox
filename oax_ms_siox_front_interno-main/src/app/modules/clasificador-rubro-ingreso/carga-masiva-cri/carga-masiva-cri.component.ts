...
@Component({
  ...
  templateUrl: './carga-masiva-cri.component.html',
  styleUrls: ['./carga-masiva-cri.component.scss'],
  standalone: true,
...
export class CargaMasivaCriComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadcrumbService.setBreadcrumbReplace([
      ...
      { nombre: 'CLASIFICADOR POR RUBRO DE INGRESO' },
      { nombre: 'CARGA MASIVA: CLASIFICADOR POR RUBRO DE INGRESO', actual: true }
    ]);
  }

    descargarEjemplo() {
      (window as any).showGlobalSpinner?.();
      this.cargaService.descargarPlantilla('1', 'PLANTILLA_EJEMPLO_CRI').subscribe({
        next: (response: any) => {
          const blob = response.body;
          const url = window.URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url;
          a.download = 'formato_ejemplo_cri.xlsx';
          a.click();
    ...
    descargarRellenar() {
      (window as any).showGlobalSpinner?.();
      this.cargaService.descargarPlantilla('1', 'PLANTILLA_RELLENAR_CRI').subscribe({
        next: (response: any) => {
          const blob = response.body;
          const url = window.URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url;
          a.download = 'formato_para_rellenar_cri.xlsx';
          a.click();
          window.URL.revokeObjectURL(url);
          (window as any).hideGlobalSpinner?.();
        },
...
const confirmationObservable = this.confirmationService.confirm({
  ...
  if (this.archivoCargado?.file && this.selectedEjercicio) {
    ...
    (window as any).showGlobalSpinner?.();
    this.cargaService.cargarArchivo(this.archivoCargado.file, this.selectedEjercicio)
      .subscribe({
        next: (res) => {
    ...
    if (inputFile) {
      inputFile.value = '';
          ...
          setTimeout(() => {
            this.router.navigateByUrl('/clasificador-rubro-ingreso/listado-cri');
