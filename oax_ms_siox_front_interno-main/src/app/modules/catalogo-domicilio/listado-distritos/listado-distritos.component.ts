...
import { CatDomicilioService, ConsultaDistritoParams } from '../../../services/cat-pais.service';

import { CatalogoDistritos } from '../../../models/catalogo-distritos.model';
import {
  ColumnDef,
...
@Component({
  selector: 'app-listado-distritos',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './listado-distritos.component.html',
  styleUrls: ['./listado-distritos.component.scss']
})
export class ListadoDistritosComponent implements OnInit {
  ...
  // Datos para la tabla
  data: CatalogoDistritos[] = [];
  paises: any[] = [];
  total = 0;

  // Columnas para la tabla de países
  colsPais: ColumnDef<any>[] = [
    { key: 'region', title: 'REGION', filter: 'text', sortable: true, width: '150px' },
    { key: 'id', title: 'CLAVE DISTRITO', filter: 'text', sortable: true, width: '100px' },
    { key: 'nombre', title: 'DISTRITO', filter: 'text', sortable: true, width: '300px' },
  ...
  ngOnInit(): void {
    this.breadcrumbService.setBreadcrumbReplace([
      ...
      { nombre: 'CONSULTA DE CATÁLOGO DE DISTRITOS', actual: true }
    ]);
  ...
  // --------- acciones de UI ---------
    ...
    buscarClasificadorPaginadoServicio(): void {
      ...
      const params: ConsultaDistritoParams = {
   ...
   if (this.filtros['id']) {
    ...
    if (this.filtros['fechaAlta']) {
      ...
      this.catDomicilioService.consultaDistrito(params).subscribe({
        next: (resp: any) => {
  ...
  exportarExcel(): void {
    const params: ConsultaDistritoParams = { page: 1, pageSize: 10000, region: this.filtros?.['region'] || '' };
    this.catDomicilioService.consultaDistrito(params).subscribe({
      next: (resp: any) => {
        const datos = resp.datos || [];
        ...
        const columnas = ['ID', 'Descripción', 'Fecha Alta'];
      ...
      columnas,
      datos: datosExcel,
      nombreArchivo: 'Reporte_Distritos.xlsx',
      tituloTabla: 'Catálogo Distritos'
    };
