...
import { CatDomicilioService, ConsultaEstadoParams, ConsultaTipoAsentamientoParams, DomicilioConsultarEstadosResponse } from '../../../services/cat-pais.service';

import { CatalogoEstado } from '../../../models/catalogo-estado.model';
import {
  ColumnDef,
...
export class ListadoEstadoComponent implements OnInit {
  ...
  colsPais: ColumnDef<any>[] = [
    { key: 'pais', title: 'PAIS', filter: 'text', sortable: true, width: '150px' },
    { key: 'id', title: 'CLAVE ESTADO', filter: 'text', sortable: true, width: '100px' },
    { key: 'estado', title: 'ESTADO', filter: 'text', sortable: true, width: '300px' },
     { key: 'clave', title: 'CLAVE CURP', filter: 'text', sortable: true, width: '300px' },
  ...
  ngOnInit(): void {
    this.breadcrumbService.setBreadcrumbReplace([
      ...
      { nombre: 'CONSULTA DE CATÁLOGO DE ESTADOS', actual: true }
    ]);
  ...
  // --------- acciones de UI ---------
    ...
    buscarClasificadorPaginadoServicio(): void {
      ...
      const params: ConsultaEstadoParams = {
   ...
   if (this.filtros['id']) {
    ...
    if (this.filtros['fechaAlta']) {
      ...
      this.catDomicilioService.consultarEstado(params).subscribe({
        next: (resp: any) => {
  ...
  exportarExcel(): void {
    const params: ConsultaEstadoParams = { page: 1, pageSize: 10000, clave: this.filtros?.['clave'] || '', pais: this.filtros?.['pais'] || '' };
    this.catDomicilioService.consultarEstado(params).subscribe({
      next: (resp: DomicilioConsultarEstadosResponse) => {
        console.log(resp);
        const datos = resp.datos || [];
        ...
        const columnas = ['PAÍS', 'CLAVE ESTADO', 'ESTADO', 'CLAVE CURP', 'FECHA ALTA'];

        const datosExcel = datos.map((item: any) => ({
          'PAÍS': item.pais || '',
          'CLAVE ESTADO': item.id || '',
          'ESTADO': item.estado?.trim() || '',
          'CLAVE CURP': item.clave || '',
        ...
        const options: ExcelReporteOptions = {
          columnas,
          datos: datosExcel,
          nombreArchivo: 'Reporte_Estados.xlsx',
          tituloTabla: 'Catálogo Estados'
        };
