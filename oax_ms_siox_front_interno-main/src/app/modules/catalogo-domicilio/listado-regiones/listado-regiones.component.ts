...
import { CatDomicilioService, ConsultaRegionParams, ConsultaTipoAsentamientoParams } from '../../../services/cat-pais.service';

import { CatalogoRegion } from '../../../models/catalogo-region.model';
import {
  ColumnDef,
...
@Component({
  ...
  templateUrl: './listado-regiones.component.html',
  styleUrls: ['./listado-regiones.component.scss']
})
export class ListadoRegionesComponent implements OnInit {
  ...
  colsPais: ColumnDef<any>[] = [
    { key: 'estado', title: 'ESTADO', filter: 'text', sortable: true, width: '150px' },
    { key: 'id', title: 'CLAVE REGION', filter: 'text', sortable: true, width: '100px' },
    { key: 'region', title: 'REGION', filter: 'text', sortable: true, width: '300px' },
  ...
  // --------- acciones de UI ---------
    ...
    buscarClasificadorPaginadoServicio(): void {
      ...
      const params: ConsultaRegionParams = {
   ...
   if (this.filtros['id']) {
    ...
    if (this.filtros['fechaAlta']) {
      ...
      this.catDomicilioService.consultarRegion(params).subscribe({
        next: (resp: any) => {
  ...
  exportarExcel(): void {
    ...
    const params: ConsultaRegionParams = { page: 1, pageSize: 10000, region: this.filtros?.['region'] || '' };
    this.catDomicilioService.consultarRegion(params).subscribe({
      next: (resp: any) => {
        const datos = resp.datos || [];
        ...
        const columnas = ['ESTADO', 'CLAVE REGIÓN','REGIÓN', 'FECHA ALTA'];

        const datosExcel = datos.map((item: any) => ({
          ESTADO: item.estado || '',
          "CLAVE REGIÓN": item.id || '',
          "REGIÓN": item.region || '',
        ...
        const options: ExcelReporteOptions = {
          columnas,
          datos: datosExcel,
          nombreArchivo: 'Reporte_Regiones.xlsx',
          tituloTabla: 'Catálogo Regiones'
        };
        ...
        const datosPdf = datos.map((item: any) => ({
          ...
          'FECHA ALTA': this.formatearFecha(item.fechaAlta)
              ...
              const options: PdfReporteOptions = {
                columnas,
                datos: datosPdf,
                nombreArchivo: 'Reporte_Regiones.pdf',
                orientacion: 'l',
                titulo: 'Catálogo Regiones'
              };
